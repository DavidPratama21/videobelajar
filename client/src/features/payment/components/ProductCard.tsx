...
const ProductCard = ({ name, price, image }: ProductCardProps) => {
  // nanti ini harus dinamis ya, jan statis
  const discount = 50;
  return (
    <div className="sm:h-fit grid gap-5 sm:gap-6 p-5 sm:p-6 bg-other-primary rounded-[10px] border border-other-border">
      <div className="grid gap-3 sm:gap-4">
        <img
          src={`/assets/images/products/${image}`}
          alt="Gambar Produk"
          className="hidden sm:inline aspect-video object-cover rounded-[10px]"
        />
        <h2 className="font-semibold text-lg text-dark-primary leading-[120%]">
          {name}
        </h2>
        {/* Price & Discount */}
